...
import { createUser, findUserByEmail, findUserByIdAndUpdate } from '#auth'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { modifiedCount } = await findUserByIdAndUpdate(event, user._id.toString(), user)
    if (!modifiedCount || modifiedCount !== 1)
      throw new AppError('We cannot update document', 'user_updare_failed', 404)
